...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  3 12:01:53 2020
...
def stress_sym(Q): #use bulk values of Q                               
    ...
    h = relax_nondim(Q)
    sigma[:,:,0] = -2*xi*(h[:,:,0]*Q[:,:,0]+h[:,:,1]*Q[:,:,1]) + 4*(Q[:,:,0]*h[:,:,0]+Q[:,:,1]*h[:,:,1])*xi*(Q[:,:,0]+0.5) - xi*h[:,:,0] - 4*L1*ddx(Q[:,:,0])**2 - 4*L1*ddx(Q[:,:,1])**2 #+ 1*zeta*Q[:,:,0]   #sigma xx
    sigma[:,:,1] =  4*(Q[:,:,0]*h[:,:,0]+Q[:,:,1]*h[:,:,1])*xi*Q[:,:,1] - 1*0.5*xi*h[:,:,1] - 4*L1*ddy(Q[:,:,0])*ddx(Q[:,:,0]) - 4*L1*ddy(Q[:,:,1])*ddx(Q[:,:,1]) #+ 1*zeta*Q[:,:,1]
    sigma[:,:,2] = -2*xi*(h[:,:,0]*Q[:,:,0]+h[:,:,1]*Q[:,:,1]) + 4*(Q[:,:,0]*h[:,:,0]+Q[:,:,1]*h[:,:,1])*xi*(-Q[:,:,0]+0.5) + xi*h[:,:,0] - 4*L1*ddy(Q[:,:,0])**2 - 4*L1*ddy(Q[:,:,1])**2 #- 1*zeta*Q[:,:,0]
    return sigma
...
def force(Q):
    ...
    temp3 = stress_sym(Q) + 1*stress_active(Q)
...
def relax_nondim(Q):
    ...
    qre[:,:,0] = -1*2*a*(1-0.5*u)*Q[:,:,0] - 1*4*a*u*Q[:,:,0]**3 - 1*4*a*u*Q[:,:,0]*Q[:,:,1]**2 + 1*L1*laplacian(Q[:,:,0])
    qre[:,:,1] = -1*2*a*(1-0.5*u)*Q[:,:,1] - 1*4*a*u*Q[:,:,1]**3 - 1*4*a*u*Q[:,:,1]*Q[:,:,0]**2 + 1*L1*laplacian(Q[:,:,1])
    return qre
...
xi = 1
zeta = -0.0008
a = 1e-4    #1e-5     #1e-8/scale**2  #lattice units
gamma = 0.02 #0.1/scale**2
L1 = 9e-4   #1e-3      #1e-6/scale**4
Q = np.zeros((N,N,2))
nx = 0.8
ny = np.sqrt(1-nx**2)
q = 0.5  #scalar order parameter
...
#dimensionless time scales#tf = N/u_max
ts = gamma/a
tn = N**2*gamma/L1
tf = N/u_max
Re = u_max*N/nu
Er = tn/tf
l = np.sqrt(L1/a)   #dimensionless correlation length, must be greater than 1
dt = 1
...
n=0
start = time.time()
print(Re, Er,l, tf, ts, tn)
#%%
T = 2.5e6 #time of integration\
start = time.time()
while t < T:
    ...
    #LC dynamics
    Q_new = Q + dt*((relax_nondim(Q)/gamma) + 1*advec_nondim(Q) + 1*S(ux,uy,Q))   #use full velcoity domain for Q evolution
...
#%%
plt.figure()    
plt.contourf(X, Y, 2*np.sqrt(Q[:,:,0]**2 + Q[:,:,1]**2), 20, cmap='RdYlBu')
plt.colorbar()
plt.quiver(X, Y, eigens(Q)[1], -eigens(Q)[2], units='xy', headlength=0, headaxislength=0, color='black')

#%%
plt.figure()    
plt.contourf(1*relax_nondim(Q)[1:N-1,1:N-1,0]/gamma +1*advec_nondim(Q)[1:N-1,1:N-1,0] + 1*S(ux,uy,Q)[1:N-1,1:N-1,0], 25, cmap='RdYlBu')
plt.colorbar()
#%%
plt.figure()  
plt.title('Q_xx')  
...
plt.figure() 
plt.title('sigma_xy')   
plt.contourf(X, Y, stress_sym(Q)[:,:,1], 25, cmap='RdYlBu')
plt.colorbar()

plt.figure()   
plt.title('sigma_yy') 
plt.contourf(X, Y, stress_sym(Q)[:,:,2], 25, cmap='RdYlBu')
plt.colorbar()

#%%
plt.figure()  
plt.title('sigma_a_xx')  
plt.contourf(X, Y, (stress_active(Q)[:,:,0]), 25, cmap='RdYlBu')
plt.colorbar()

plt.figure() 
plt.title('sigma_a_xy')   
plt.contourf(X, Y, stress_active(Q)[:,:,1], 25, cmap='RdYlBu')
plt.colorbar()

plt.figure()   
plt.title('sigma_a_yy') 
plt.contourf(X, Y, stress_active(Q)[:,:,2], 25, cmap='RdYlBu')
plt.colorbar()
#%%
plt.figure()  
plt.title('ddx(sigma_xx)')  
plt.contourf(X, Y, ddx(0*stress_active(Q)[:,:,0]+1*stress_sym(Q)[:,:,0]), 25, cmap='RdYlBu')
plt.colorbar()

plt.figure()  
plt.title('ddy(sigma_xx)')  
plt.contourf(X, Y, ddy(0*stress_active(Q)[:,:,0]+1*stress_sym(Q)[:,:,0]), 25, cmap='RdYlBu')
plt.colorbar()
#%%
plt.figure() 
plt.title('sigma_xy')   
